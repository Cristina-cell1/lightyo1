/**
 * Fightzone Stripe + Supabase Worker
 * ---------------------------------
 * Endpoints:
 *  - GET  /api/ping            -> "ok"
 *  - GET  /api/streams-sync    -> fetches ppv streams and normalizes for Fightzone
 *  - POST /api/checkout        -> creates Stripe Checkout Session, returns { url }
 *  - POST /api/stripe-webhook  -> Stripe webhook (checkout.session.completed), updates Supabase
 *
 * Cloudflare env (Variables/Secrets):
 *  - SITE_URL (plaintext)                     e.g. https://fightzone.com or http://127.0.0.1:5500
 *  - SUPABASE_URL (plaintext)                 https://xxxx.supabase.co
 *  - SUPABASE_ANON_KEY (plaintext)
 *  - SUPABASE_SERVICE_ROLE_KEY (SECRET)       never expose in frontend
 *  - STRIPE_SECRET_KEY (SECRET)               sk_test_... or sk_live_...
 *  - STRIPE_WEBHOOK_SECRET (SECRET)           whsec_...
 *    Optional:
 *  - STRIPE_WEBHOOK_SECRET_ALT (SECRET)
 *  - ALLOW_ORIGIN (plaintext)                 optional fixed allowed origin
 *
 *  - STRIPE_PRICE_ONE_TIME (plaintext)        price_...
 *  - STRIPE_PRICE_PASS (plaintext)            price_...
 *  - STRIPE_PRICE_ELITE (plaintext)           price_...
 *
 * IMPORTANT:
 *  - one_time uses mode="payment"
 *  - pass uses mode="subscription"
 *  - elite uses mode="payment" (lifetime one-time)
 */

const JSON_HEADERS = { "content-type": "application/json; charset=utf-8" };

function json(data, status = 200, extraHeaders = {}) {
  return new Response(JSON.stringify(data), {
    status,
    headers: { ...JSON_HEADERS, ...extraHeaders },
  });
}

function readOrigin(request, env) {
  const origin = request.headers.get("Origin") || "";

  const allowed = [
  "http://localhost:3000",
  "http://127.0.0.1:3000",
  "http://localhost:5500",
  "http://127.0.0.1:5500",
  "https://fzpv.st",
  "https://www.fzpv.st",
  "https://tiny-glitter-8d9e.solfetka2345.workers.dev"
  ];
  
  if (allowed.includes(origin)) return origin;

  return "https://tiny-glitter-8d9e.solfetka2345.workers.dev";
}

function corsHeaders(request, env) {
  const allowOrigin = readOrigin(request, env);
  return {
    "access-control-allow-origin": allowOrigin,
    "access-control-allow-methods": "GET,POST,OPTIONS",
    "access-control-allow-headers": "content-type, authorization, stripe-signature, x-fz-client-key",
    "access-control-max-age": "86400",
  };
}

function withCors(request, env, res) {
  return new Response(res.body, {
    status: res.status,
    headers: {
      ...Object.fromEntries(res.headers),
      ...corsHeaders(request, env),
    },
  });
}

function requireEnv(env, keys) {
  for (const k of keys) {
    if (!env[k]) throw new Error(`Missing env: ${k}`);
  }
}

async function stripeRequest(env, path, opts = {}) {
  const url = "https://api.stripe.com/v1" + path;
  const headers = {
    Authorization: `Bearer ${env.STRIPE_SECRET_KEY}`,
    "Content-Type": "application/x-www-form-urlencoded",
    ...(opts.headers || {}),
  };

  const res = await fetch(url, { ...opts, headers });
  const text = await res.text();

  let data;
  try {
    data = JSON.parse(text);
  } catch {
    data = { raw: text };
  }

  if (!res.ok) {
    const msg = data?.error?.message || `Stripe error (${res.status})`;
    throw new Error(msg);
  }

  return data;
}

/* -------------------- Generic fetch helpers -------------------- */

async function fetchJson(url, init = {}) {
  const res = await fetch(url, init);
  if (!res.ok) {
    throw new Error(`Fetch failed ${res.status}: ${url}`);
  }
  return await res.json();
}

function uniqueStrings(arr) {
  return [...new Set((Array.isArray(arr) ? arr : []).filter(Boolean).map((x) => String(x).trim()))];
}

/* -------------------- PPV API helpers -------------------- */

async function getPpvStreams() {
  const DOMAINS = ["api.ppv.to"];
  let lastErr = null;

  for (const domain of DOMAINS) {
    try {
      const data = await fetchJson(`https://${domain}/api/streams`);
      if (data?.success && Array.isArray(data?.streams)) {
        return { source_domain: domain, data };
      }
    } catch (e) {
      lastErr = e;
    }
  }

  throw lastErr || new Error("No working PPV domains");
}


function normalizeFightzoneStreams(apiData) {
  const out = [];
  const groups = Array.isArray(apiData?.streams) ? apiData.streams : [];
  const nowMs = Date.now();

  for (const group of groups) {
    const category = String(group?.category || "").trim();
    const categoryId = group?.id ?? null;
    const groupAlwaysLive = Number(group?.always_live || 0);

    const streams = Array.isArray(group?.streams) ? group.streams : [];
    for (const s of streams) {
      const uri = String(s?.uri_name || "").trim();
      if (!uri) continue;

      const startsAt = Number(s?.starts_at || 0);
      const endsAt = Number(s?.ends_at || 0);
      const startsMs = startsAt > 0 ? startsAt * 1000 : 0;
      const endsMs = endsAt > 0 ? endsAt * 1000 : 0;
      const alwaysLive = Number(s?.always_live || groupAlwaysLive || 0) === 1;

      const cat = [
  category,
  s?.category_name,
  s?.name,
  s?.uri_name
].join(" ").toLowerCase();

const isForcedFree =
  Number(s?.always_live || groupAlwaysLive || 0) === 1;

out.push({
  category,
  category_id: categoryId,
  id: s?.id ?? null,
  name: String(s?.name || "").trim(),
  title: String(s?.name || "").trim(),
  tag: String(s?.tag || "").trim(),
  poster: String(s?.poster || "").trim(),
  uri_name: uri,
  starts_at: startsAt,
  ends_at: endsAt,
  always_live: alwaysLive ? 1 : 0,
  category_name: String(s?.category_name || category || "").trim(),
  allowpaststreams: Number(s?.allowpaststreams || 0),
  iframe_url: s?.iframe || `https://pooembed.eu/embed/${uri}`,

  // 👇 ВОТ ЭТО ГЛАВНОЕ
  free: isForcedFree ? 1 : 0,

  is_live: alwaysLive || (startsMs > 0 && endsMs > 0 && nowMs >= startsMs && nowMs <= endsMs),
  is_ended: !alwaysLive && endsMs > 0 && nowMs > endsMs,
});
    }
  }

  return out;
}

function filterFightzoneStreams(events, url) {
  const categoryParam = url.searchParams.get("category");
  const q = (url.searchParams.get("q") || "").trim().toLowerCase();
  const includeEnded = url.searchParams.get("includeEnded") === "1";

  let filtered = events;

  if (categoryParam) {
    const wanted = uniqueStrings(categoryParam.split(",").map((x) => x.toLowerCase()));
    filtered = filtered.filter((e) => wanted.includes(String(e.category || "").toLowerCase()));
  }

  if (q) {
    filtered = filtered.filter((e) => {
      const hay = [
        e.title,
        e.tag,
        e.category,
        e.category_name,
        e.uri_name,
      ]
        .join(" ")
        .toLowerCase();

      return hay.includes(q);
    });
  }

if (!includeEnded) {
    filtered = filtered.filter((e) => !e.is_ended);
}

filtered.sort((a, b) => {
  const aStart = Number(a.starts_at || 0);
  const bStart = Number(b.starts_at || 0);
  return aStart - bStart;
});


const FOOTBALL_LIMIT = 18;

const football = [];
const darts = [];
const americanFootball = [];
const baseball = [];
const streams247 = [];
const other = [];

for (const e of filtered) {
  const cat = [
  e.category,
  e.category_name,
  e.title,
  e.name,
  e.uri_name
  ].join(" ").toLowerCase();

  if (
  Number(e.always_live) === 1 ||
  cat.includes("24/7") ||
  cat.includes("south park") ||
  cat.includes("family guy") ||
  cat.includes("simpsons") ||
  cat.includes("spongebob") ||
  cat.includes("rally")
 ) {
  streams247.push(e);
 }

  else if (cat.includes("american football")) {
    americanFootball.push(e);
  }

  else if (
  cat.includes("darts") ||
  cat.includes("dart") ||
  cat.includes("pdc") ||
  cat.includes("players championship")
  ) {
  darts.push(e);
  }

  else if (cat.includes("baseball")) {
    baseball.push(e);
  }

  else if (
    cat.includes("football") ||
    cat.includes("soccer") ||
    cat.includes("premier league") ||
    cat.includes("laliga") ||
    cat.includes("la liga") ||
    cat.includes("serie a") ||
    cat.includes("bundesliga") ||
    cat.includes("champions league") ||
    cat.includes("uefa") ||
    cat.includes("europa league") ||
    cat.includes("conference league") ||
    cat.includes("fa cup") ||
    cat.includes("carabao cup") ||
    cat.includes("league cup") ||
    cat.includes("copa del rey") ||
    cat.includes("mls") ||
    cat.includes("ligue 1") ||
    cat.includes("fifa") ||
    cat.includes("world cup") ||
    cat.includes("nations league")
  ) {
    football.push(e);
  }

  else {
    other.push(e);
  }
}

function sortWithPosterFirst(arr) {
  arr.sort((a, b) => {
    const aHasPoster = String(a.poster || "").trim() ? 0 : 1;
    const bHasPoster = String(b.poster || "").trim() ? 0 : 1;

    if (aHasPoster !== bHasPoster) {
      return aHasPoster - bHasPoster;
    }

    const aStart = Number(a.starts_at || 0);
    const bStart = Number(b.starts_at || 0);
    return aStart - bStart;
  });
}

sortWithPosterFirst(football);
sortWithPosterFirst(darts);
sortWithPosterFirst(americanFootball);
sortWithPosterFirst(baseball);
sortWithPosterFirst(streams247);

filtered = [
  ...other,
  ...football.slice(0, FOOTBALL_LIMIT),

  // новые секции — без лимита
  ...darts,
  ...americanFootball,
  ...baseball,
  ...streams247,
];

return filtered;
}

async function getAllOverrides(env) {
  const [iframeRaw, dateRaw] = await Promise.all([
    env.IFRAME_OVERRIDES.get('__all__'),
    env.DATE_OVERRIDE.get('__all__'),
  ]);
  return {
    iframeMap: JSON.parse(iframeRaw || '{}'),
    dateMap:   JSON.parse(dateRaw  || '{}'),
  };
}

async function applyOverridesToEvents(env, events) {
  const { iframeMap, dateMap } = await getAllOverrides(env);

  return events.map(ev => {
    const key = String(ev?.uri_name || ev?.id || '').trim();
    return {
      ...ev,
      iframe_override: key ? (iframeMap[key] ?? null) : null,
      date_override:   key ? (dateMap[key]   ?? null) : null,
    };
  });
}

/* -------------------- Stripe signature verify -------------------- */

async function verifyStripeSignature(rawBody, sigHeader, webhookSecret) {
  if (!sigHeader || !webhookSecret) return false;

  const parts = sigHeader.split(",").map((s) => s.trim());
  const tPart = parts.find((p) => p.startsWith("t="));
  const v1Parts = parts.filter((p) => p.startsWith("v1="));
  if (!tPart || v1Parts.length === 0) return false;

  const timestamp = tPart.slice(2);
  const signedPayload = `${timestamp}.${rawBody}`;

  const key = await crypto.subtle.importKey(
    "raw",
    new TextEncoder().encode(webhookSecret),
    { name: "HMAC", hash: "SHA-256" },
    false,
    ["sign"]
  );

  const sigBuf = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(signedPayload));
  const hex = [...new Uint8Array(sigBuf)].map((b) => b.toString(16).padStart(2, "0")).join("");

  for (const v1Part of v1Parts) {
    const v1 = v1Part.slice(3);
    if (timingSafeEqual(hex, v1)) return true;
  }

  return false;
}

function timingSafeEqual(a, b) {
  if (typeof a !== "string" || typeof b !== "string") return false;
  if (a.length !== b.length) return false;

  let out = 0;
  for (let i = 0; i < a.length; i++) {
    out |= a.charCodeAt(i) ^ b.charCodeAt(i);
  }
  return out === 0;
}

/* -------------------- Supabase helpers -------------------- */

async function getSupabaseUser(env, accessToken) {
  const res = await fetch(`${env.SUPABASE_URL}/auth/v1/user`, {
    headers: {
      apikey: env.SUPABASE_ANON_KEY,
      Authorization: `Bearer ${accessToken}`,
    },
  });

  if (!res.ok) return null;
  return await res.json();
}


async function sha256Hex(text) {
  const data = new TextEncoder().encode(text);
  const hashBuffer = await crypto.subtle.digest("SHA-256", data);

  return [...new Uint8Array(hashBuffer)]
    .map(b => b.toString(16).padStart(2, "0"))
    .join("");
}

async function recordCodeSuccessNumber(env, userId, codeName = "main_code") {
  requireEnv(env, [
    "SUPABASE_URL",
    "SUPABASE_SERVICE_ROLE_KEY"
  ]);

  const secret = env.CODE_HASH_SECRET || "fallback_code_secret";
  const userHash = await sha256Hex(`${userId}:${secret}`);

  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/code_success_unique`, {
    method: "POST",
    headers: {
      apikey: env.SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
      "Content-Type": "application/json",
      Prefer: "resolution=ignore-duplicates,return=minimal"
    },
    body: JSON.stringify({
      code_name: codeName,
      user_hash: userHash
    })
  });

  if (!res.ok) {
    throw new Error("Failed to record code success: " + await res.text());
  }
}

async function supabaseInsertEventAccess(env, user_id, event_id) {
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/event_access`, {
    method: "POST",
    headers: {
      apikey: env.SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
      "Content-Type": "application/json",
      Prefer: "resolution=merge-duplicates,return=minimal",
    },
    body: JSON.stringify({ user_id, event_id }),
  });

  if (!res.ok) {
    throw new Error("Supabase insert event_access failed: " + (await res.text()));
  }
}

async function supabaseExtendPass(env, user_id) {
  const iso = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();

  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/profiles?id=eq.${user_id}`, {
    method: "PATCH",
    headers: {
      apikey: env.SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
      "Content-Type": "application/json",
      Prefer: "return=minimal",
    },
    body: JSON.stringify({ pass_until: iso }),
  });

  if (!res.ok) {
    throw new Error("Supabase update pass_until failed: " + (await res.text()));
  }
}

async function supabaseSetElite(env, user_id) {
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/profiles?id=eq.${user_id}`, {
    method: "PATCH",
    headers: {
      apikey: env.SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
      "Content-Type": "application/json",
      Prefer: "return=minimal",
    },
    body: JSON.stringify({ is_elite: true }),
  });

  if (!res.ok) {
    throw new Error("Supabase set elite failed: " + (await res.text()));
  }
}

async function getDiscordProfileByUserId(env, user_id) {
  const res = await fetch(
    `${env.SUPABASE_URL}/rest/v1/profiles?id=eq.${encodeURIComponent(user_id)}&select=id,discord_id,discord_access_token,discord_refresh_token,discord_token_expires_at`,
    {
      headers: {
        apikey: env.SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
      },
    }
  );

  if (!res.ok) {
    throw new Error("Failed to load Discord profile: " + (await res.text()));
  }

  const rows = await res.json();
  return rows?.[0] || null;
}

async function updateDiscordTokens(env, user_id, patch) {
  const res = await fetch(
    `${env.SUPABASE_URL}/rest/v1/profiles?id=eq.${encodeURIComponent(user_id)}`,
    {
      method: "PATCH",
      headers: {
        apikey: env.SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
        "Content-Type": "application/json",
        Prefer: "return=minimal",
      },
      body: JSON.stringify(patch),
    }
  );

  if (!res.ok) {
    throw new Error("Failed to update Discord tokens: " + (await res.text()));
  }
}

async function refreshDiscordToken(env, refreshToken) {
  const body = new URLSearchParams();
  body.set("client_id", env.DISCORD_CLIENT_ID);
  body.set("client_secret", env.DISCORD_CLIENT_SECRET);
  body.set("grant_type", "refresh_token");
  body.set("refresh_token", refreshToken);

  const res = await fetch("https://discord.com/api/v10/oauth2/token", {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
    },
    body,
  });

  const data = await res.json();

  if (!res.ok) {
    throw new Error("Discord refresh failed: " + JSON.stringify(data));
  }

  return data;
}

async function ensureFreshDiscordAccessToken(env, profile) {
  const now = Math.floor(Date.now() / 1000);

  if (
    profile.discord_access_token &&
    profile.discord_token_expires_at &&
    Number(profile.discord_token_expires_at) - 60 > now
  ) {
    return profile.discord_access_token;
  }

  if (!profile.discord_refresh_token) {
    throw new Error("User has no discord_refresh_token");
  }

  const tokenData = await refreshDiscordToken(env, profile.discord_refresh_token);

  await updateDiscordTokens(env, profile.id, {
    discord_access_token: tokenData.access_token,
    discord_refresh_token: tokenData.refresh_token || profile.discord_refresh_token,
    discord_token_expires_at: now + Number(tokenData.expires_in || 0),
  });

  return tokenData.access_token;
}

async function addDiscordUserToGuild(env, discordUserId, userAccessToken) {
  const res = await fetch(
    `https://discord.com/api/v10/guilds/${env.DISCORD_GUILD_ID}/members/${discordUserId}`,
    {
      method: "PUT",
      headers: {
        Authorization: `Bot ${env.DISCORD_BOT_TOKEN}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        access_token: userAccessToken,
      }),
    }
  );

  if (![201, 204].includes(res.status)) {
    throw new Error("Add guild member failed: " + res.status + " " + (await res.text()));
  }
}

async function addDiscordRole(env, discordUserId, roleId) {
  const res = await fetch(
    `https://discord.com/api/v10/guilds/${env.DISCORD_GUILD_ID}/members/${discordUserId}/roles/${roleId}`,
    {
      method: "PUT",
      headers: {
        Authorization: `Bot ${env.DISCORD_BOT_TOKEN}`,
      },
    }
  );

  if (res.status !== 204) {
    throw new Error("Add role failed: " + res.status + " " + (await res.text()));
  }
}

async function syncDiscordRoleForPurchase(env, user_id, plan) {
  if (!["pass", "elite"].includes(plan)) return;

  const profile = await getDiscordProfileByUserId(env, user_id);

  if (!profile?.discord_id) {
    console.log(`Discord not connected for user ${user_id}, skipping role sync`);
    return;
  }

  const accessToken = await ensureFreshDiscordAccessToken(env, profile);

  await addDiscordUserToGuild(env, profile.discord_id, accessToken);

  if (plan === "pass") {
    await addDiscordRole(env, profile.discord_id, env.DISCORD_SUB_ROLE_ID);
    return;
  }

  if (plan === "elite") {
    await addDiscordRole(env, profile.discord_id, env.DISCORD_ELITE_ROLE_ID);
  }
}
/* -------------------- Handlers -------------------- */
async function getProfileByUserId(env, userId) {
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/profiles?id=eq.${userId}&select=id,is_admin`, {
    headers: {
      apikey: env.SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
    },
  });

  if (!res.ok) {
    throw new Error("Failed to load profile");
  }

  const rows = await res.json();
  return rows?.[0] || null;
}
async function requireAdmin(request, env) {
  const auth = request.headers.get("Authorization") || "";
  const token = auth.startsWith("Bearer ") ? auth.slice(7) : null;

  if (!token) {
    return { ok: false, response: json({ error: "Missing token" }, 401) };
  }

  const user = await getSupabaseUser(env, token);
  if (!user?.id) {
    return { ok: false, response: json({ error: "Invalid user" }, 401) };
  }

  const profile = await getProfileByUserId(env, user.id);
  if (!profile?.is_admin) {
    return { ok: false, response: json({ error: "Admin only" }, 403) };
  }

  return { ok: true };
}

async function handleGetIframeOverride(request, env) {
  const url     = new URL(request.url);
  const eventId = String(url.searchParams.get('eventId') || '').trim();
  if (!eventId) return json({ error: 'Missing eventId' }, 400);

  const { iframeMap } = await getAllOverrides(env);
  return json({ ok: true, eventId, iframe: iframeMap[eventId] ?? null });
}

async function handleSetIframeOverride(request, env) {
  const admin = await requireAdmin(request, env);
  if (!admin.ok) return admin.response;

  const body    = await request.json().catch(() => ({}));
  const eventId = String(body.eventId || '').trim();
  const iframe  = String(body.iframe  || '').trim();
  if (!eventId || !iframe) return json({ error: 'Missing data' }, 400);

  const raw = await env.IFRAME_OVERRIDES.get('__all__');
  const map = JSON.parse(raw || '{}');
  map[eventId] = iframe;
  await env.IFRAME_OVERRIDES.put('__all__', JSON.stringify(map));

  return json({ ok: true });
}

async function handleDeleteIframeOverride(request, env) {
  const admin = await requireAdmin(request, env);
  if (!admin.ok) return admin.response;

  const body    = await request.json().catch(() => ({}));
  const eventId = String(body.eventId || '').trim();
  if (!eventId) return json({ error: 'Missing eventId' }, 400);

  const raw = await env.IFRAME_OVERRIDES.get('__all__');
  const map = JSON.parse(raw || '{}');
  delete map[eventId];
  await env.IFRAME_OVERRIDES.put('__all__', JSON.stringify(map));

  return json({ ok: true });
}



async function handleCheckout(request, env) {
  requireEnv(env, [
    "STRIPE_SECRET_KEY",
    "SUPABASE_URL",
    "SUPABASE_ANON_KEY",
  ]);

  const auth = request.headers.get("Authorization") || "";
  const token = auth.startsWith("Bearer ") ? auth.slice(7) : null;
  if (!token) return json({ error: "Missing Authorization Bearer token" }, 401);

  const body = await request.json().catch(() => ({}));
  const plan = String(body.plan || "").trim();
  const event_id = String(body.event_id || "").trim();
  const return_to = String(body.return_to || env.SITE_URL || "").trim();

  if (!["one_time", "pass", "elite"].includes(plan)) {
    return json({ error: "Invalid plan" }, 400);
  }

  if (plan === "one_time" && !event_id) {
    return json({ error: "Missing event_id for one_time" }, 400);
  }

  if (!return_to) {
    return json({ error: "Missing SITE_URL/return_to" }, 400);
  }

  const user = await getSupabaseUser(env, token);
  if (!user?.id) {
    return json({ error: "Invalid user session" }, 401);
  }

  const price =
    plan === "one_time"
      ? env.STRIPE_PRICE_ONE_TIME
      : plan === "pass"
        ? env.STRIPE_PRICE_PASS
        : env.STRIPE_PRICE_ELITE;

  if (!price) {
    return json({ error: "Missing STRIPE_PRICE_* in env" }, 500);
  }

  // pass = monthly subscription
  // elite = lifetime one-time
  const mode = plan === "pass" ? "subscription" : "payment";

  const successUrl = new URL(return_to);
  successUrl.searchParams.set("paid", "1");
  successUrl.searchParams.set("plan", plan);
  if (event_id) successUrl.searchParams.set("event_id", event_id);

  const cancelUrl = new URL(return_to);
  cancelUrl.searchParams.set("paid", "0");
  cancelUrl.searchParams.set("plan", plan);
  if (event_id) cancelUrl.searchParams.set("event_id", event_id);

  const form = new URLSearchParams();
  form.set("mode", mode);
  form.set("success_url", successUrl.toString());
  form.set("cancel_url", cancelUrl.toString());
  form.set("client_reference_id", user.id);
  form.set("line_items[0][price]", price);
  form.set("line_items[0][quantity]", "1");

  form.set("metadata[user_id]", user.id);
  form.set("metadata[plan]", plan);
  form.set("metadata[event_id]", event_id || "");

  if (user?.email) {
    form.set("customer_email", user.email);
  }

  const session = await stripeRequest(env, "/checkout/sessions", {
    method: "POST",
    body: form.toString(),
  });

  if (!session?.url) {
    throw new Error("Stripe session has no url");
  }

  return json({ url: session.url }, 200);
}

async function handleStripeWebhook(request, env) {
  requireEnv(env, [
    "STRIPE_WEBHOOK_SECRET",
    "SUPABASE_URL",
    "SUPABASE_SERVICE_ROLE_KEY",
    "DISCORD_BOT_TOKEN",
    "DISCORD_GUILD_ID",
    "DISCORD_SUB_ROLE_ID",
    "DISCORD_ELITE_ROLE_ID",
    "DISCORD_CLIENT_ID",
    "DISCORD_CLIENT_SECRET",
  ]);

  const sig =
    request.headers.get("Stripe-Signature") ||
    request.headers.get("stripe-signature");

  const rawBody = await request.text();

  let ok = await verifyStripeSignature(rawBody, sig, env.STRIPE_WEBHOOK_SECRET);
  if (!ok && env.STRIPE_WEBHOOK_SECRET_ALT) {
    ok = await verifyStripeSignature(rawBody, sig, env.STRIPE_WEBHOOK_SECRET_ALT);
  }

  if (!ok) {
    return new Response("Invalid signature", { status: 400 });
  }

  let event;
  try {
    event = JSON.parse(rawBody);
  } catch {
    return new Response("Invalid JSON", { status: 400 });
  }

  if (event.type !== "checkout.session.completed") {
    return new Response("ok", { status: 200 });
  }

  const s = event.data?.object || {};
  const md = s.metadata || {};
  const user_id = String(md.user_id || "").trim();
  const plan = String(md.plan || "").trim();
  const event_id = String(md.event_id || "").trim();

  if (!user_id || !plan) {
    return new Response("ok", { status: 200 });
  }

  if (plan === "one_time") {
    if (event_id) await supabaseInsertEventAccess(env, user_id, event_id);
    return new Response("ok", { status: 200 });
  }

  if (plan === "pass") {
    await supabaseExtendPass(env, user_id);
    await syncDiscordRoleForPurchase(env, user_id, "pass");
    return new Response("ok", { status: 200 });
  }

  if (plan === "elite") {
    await supabaseSetElite(env, user_id);
    await syncDiscordRoleForPurchase(env, user_id, "elite");
    return new Response("ok", { status: 200 });
  }

  return new Response("ok", { status: 200 });
}

async function handleStreamsSync(request, env, ctx) {
  const url = new URL(request.url);
  const cache = caches.default;

  // Ключ кеша: только URL + query
  const cacheKey = new Request(request.url, request);

  async function buildFreshResponse() {
    const { source_domain, data } = await getPpvStreams();
    const normalized = normalizeFightzoneStreams(data);
    const filtered = filterFightzoneStreams(normalized, url);
    const events = await applyOverridesToEvents(env, filtered);

    return json(
      {
        success: true,
        source_domain,
        timestamp: Math.floor(Date.now() / 1000),
        count: events.length,
        events,
      },
      200,
      {
        // 60 сек свежий, потом можно отдать старый и обновить в фоне
        "Cache-Control": "public, max-age=60, stale-while-revalidate=300",
      }
    );
  }

const cached = await cache.match(cacheKey);
if (cached) {
  const age = parseInt(cached.headers.get('Age') || '0', 10);
  if (age > 50) { // обновляем только если старше 50 сек
    ctx.waitUntil((async () => {
      try {
        const fresh = await buildFreshResponse();
        await cache.put(cacheKey, fresh.clone());
      } catch (err) {
        console.log("SWR refresh failed:", String(err?.message || err));
      }
    })());
  }
  return cached;
}

  // 3. Если кеша нет — тянем свежие данные
try {
  const fresh = await buildFreshResponse();
  ctx.waitUntil(cache.put(cacheKey, fresh.clone()));
  return fresh;
} catch (err) {
  console.log("Initial fetch failed:", err);

  // если API умер — возвращаем старый кеш (если вдруг появился)
  const fallback = await cache.match(cacheKey);
  if (fallback) return fallback;

  throw err;
 }
}

async function handleGetDateOverride(request, env) {
  const url     = new URL(request.url);
  const eventId = String(url.searchParams.get('eventId') || '').trim();
  if (!eventId) return json({ error: 'Missing eventId' }, 400);

  const { dateMap } = await getAllOverrides(env);
  return json({ ok: true, eventId, date: dateMap[eventId] ?? null });
}

async function handleSetDateOverride(request, env) {
  const admin = await requireAdmin(request, env);
  if (!admin.ok) return admin.response;

  const body    = await request.json().catch(() => ({}));
  const eventId = String(body.eventId || '').trim();
  const date    = String(body.date    || '').trim();
  if (!eventId || !date) return json({ error: 'Missing data' }, 400);

  const raw = await env.DATE_OVERRIDE.get('__all__');
  const map = JSON.parse(raw || '{}');
  map[eventId] = date;
  await env.DATE_OVERRIDE.put('__all__', JSON.stringify(map));

  return json({ ok: true });
}

async function handleDeleteDateOverride(request, env) {
  const admin = await requireAdmin(request, env);
  if (!admin.ok) return admin.response;

  const body    = await request.json().catch(() => ({}));
  const eventId = String(body.eventId || '').trim();
  if (!eventId) return json({ error: 'Missing eventId' }, 400);

  const raw = await env.DATE_OVERRIDE.get('__all__');
  const map = JSON.parse(raw || '{}');
  delete map[eventId];
  await env.DATE_OVERRIDE.put('__all__', JSON.stringify(map));

  return json({ ok: true });
}

/* -------------------- Worker entry -------------------- */

export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);

    const MAIN_SITE = "https://fzpv.st";
    const CLIENT_KEY = "fz_site_v1_8Qk29LmX";

    if (request.method === "OPTIONS") {
      return new Response(null, { headers: corsHeaders(request, env) });
    }

    if (url.pathname === "/") {
      return Response.redirect(MAIN_SITE, 302);
    }

    if (url.pathname === "/api/ping") {
      return new Response("ok", { status: 200 });
    }

try {

if (url.pathname === "/api/env-check" && request.method === "GET") {
  return json({
    ok: true,
    worker: "fightzone-worker",
    has_SUPABASE_URL: !!env.SUPABASE_URL,
    has_SUPABASE_ANON_KEY: !!env.SUPABASE_ANON_KEY,
    has_SUPABASE_SERVICE_ROLE_KEY: !!env.SUPABASE_SERVICE_ROLE_KEY,
    has_CODE_HASH_SECRET: !!env.CODE_HASH_SECRET,
    supabase_url_start: env.SUPABASE_URL ? env.SUPABASE_URL.slice(0, 35) : null
  });
}

if (url.pathname === "/api/client.js" && request.method === "GET") {
  requireEnv(env, [
    "SUPABASE_URL",
    "SUPABASE_ANON_KEY"
  ]);

  const res = await fetch("https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2");
  const supabaseLib = await res.text();

  const js = `
${supabaseLib}

(function(){
  window.FZ_API_BASE = window.FZ_API_BASE || "https://api.fzpv.st";

  if (!window.supabaseClient && window.supabase && window.supabase.createClient) {
    window.supabaseClient = window.supabase.createClient(
      ${JSON.stringify(env.SUPABASE_URL)},
      ${JSON.stringify(env.SUPABASE_ANON_KEY)}
    );

    window.sb = window.supabaseClient;
  }

  window.dispatchEvent(new Event("fz:supabase-ready"));
})();
`;

  return new Response(js, {
    status: 200,
    headers: {
      "Content-Type": "application/javascript; charset=utf-8",
      "Cache-Control": "public, max-age=300",
      ...corsHeaders(request, env),
    },
  });
}



if (url.pathname === "/api/streams-sync" && request.method === "GET") {
  const accept = request.headers.get("Accept") || "";
  const secFetchMode = request.headers.get("Sec-Fetch-Mode") || "";

  const isBrowserNavigation =
    secFetchMode === "navigate" ||
    accept.includes("text/html");

  if (isBrowserNavigation) {
    return Response.redirect(MAIN_SITE, 302);
  }

  return withCors(request, env, await handleStreamsSync(request, env, ctx));
}

if (url.pathname === "/api/checkout" && request.method === "POST") {
        return withCors(request, env, await handleCheckout(request, env));
}

if (url.pathname === "/api/stripe-webhook" && request.method === "POST") {
   const res = await handleStripeWebhook(request, env);
   return withCors(request, env, res);
}


if (url.pathname === "/api/code-success" && request.method === "POST") {
  const auth = request.headers.get("Authorization") || "";
  const token = auth.startsWith("Bearer ") ? auth.slice(7) : null;

  if (!token) {
    return withCors(request, env, json({ ok: false, error: "no_token" }, 401));
  }

  const user = await getSupabaseUser(env, token);

  if (!user?.id) {
    return withCors(request, env, json({ ok: false, error: "invalid_user" }, 401));
  }

  const body = await request.json().catch(() => ({}));
  const codeName = String(body.code_name || "main_code").trim() || "main_code";

  await recordCodeSuccessNumber(env, user.id, codeName);

  return withCors(request, env, json({ ok: true }));
}

if (url.pathname === "/api/check-access" && request.method === "GET") {
  const auth = request.headers.get("Authorization") || "";
  const token = auth.startsWith("Bearer ") ? auth.slice(7) : null;

  if (!token) {
    return withCors(request, env, json({ access: false, reason: "no_token" }));
  }

  const user = await getSupabaseUser(env, token);
  if (!user?.id) {
    return withCors(request, env, json({ access: false, reason: "invalid_user" }));
  }

  const event_id = url.searchParams.get("event_id") || "";

  // 👉 получаем профиль
  const profileRes = await fetch(`${env.SUPABASE_URL}/rest/v1/profiles?id=eq.${user.id}`, {
    headers: {
      apikey: env.SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
    },
  });

  const profile = await profileRes.json().then(r => r?.[0] || null);

  // elite
  if (profile?.is_elite) {
    return withCors(request, env, json({ access: true, type: "elite" }));
  }

  // pass
  if (profile?.pass_until && new Date(profile.pass_until) > new Date()) {
    return withCors(request, env, json({ access: true, type: "pass" }));
  }

  // one_time
  if (event_id) {
    const evRes = await fetch(
      `${env.SUPABASE_URL}/rest/v1/event_access?user_id=eq.${user.id}&event_id=eq.${event_id}`,
      {
        headers: {
          apikey: env.SUPABASE_SERVICE_ROLE_KEY,
          Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`,
        },
      }
    );

    const evData = await evRes.json();
    if (Array.isArray(evData) && evData.length > 0) {
      return withCors(request, env, json({ access: true, type: "one_time" }));
    }
  }

  return withCors(request, env, json({ access: false, reason: "no_access" }));
}

if (url.pathname === "/api/iframe-override" && request.method === "GET") {
  return withCors(request, env, await handleGetIframeOverride(request, env));
}

if (url.pathname === "/api/iframe-override" && request.method === "POST") {
  return withCors(request, env, await handleSetIframeOverride(request, env));
}

if (url.pathname === "/api/iframe-override" && request.method === "DELETE") {
  return withCors(request, env, await handleDeleteIframeOverride(request, env));
}

if (url.pathname === "/api/date-override" && request.method === "GET") {
  return withCors(request, env, await handleGetDateOverride(request, env));
}

if (url.pathname === "/api/date-override" && request.method === "POST") {
  return withCors(request, env, await handleSetDateOverride(request, env));
}

if (url.pathname === "/api/date-override" && request.method === "DELETE") {
  return withCors(request, env, await handleDeleteDateOverride(request, env));
}

return new Response("Not found", { status: 404 });

} catch (e) {
      console.log("WORKER ERROR:", String(e?.message || e));
      return withCors(request, env, json({ error: String(e?.message || e) }, 500));
    }
  },
};